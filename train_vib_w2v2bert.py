...
# ---------------- VIB -------------------
class VIB(nn.Module):
    def __init__(self, d_in: int, k_lat: int = 256,vib_std_floor:float = 0.0):
        super().__init__()
        self.mu     = nn.Linear(d_in, k_lat)
        self.logvar = nn.Linear(d_in, k_lat)
        self.vib_std_floor = vib_std_floor
    ...
    def forward(self, h, train=True):
        ...
        if train:
            ...
            vib_std_floor = self.vib_std_floor
            #-------Noise floor clamp-------
            if vib_std_floor>0:
                std_eff = torch.clamp(std,min=vib_std_floor)
                logvar_eff = 2.0 * torch.log(std_eff+1e-8)
            else:
                std_eff = std
                logvar_eff = logvar

            eps = torch.randn_like(std_eff)
            z = mu + eps * std_eff
        else:
            z = mu
        kl = -0.5 * torch.sum(1 + logvar_eff - mu.pow(2) - logvar_eff.exp(), dim=-1)
        return z, kl
...
class PrivacyNet(nn.Module):
    def __init__(self, base, n_spk, *, use_grl, use_vib, vib_lat, null_rank,vib_std_floor):
        ...
        if use_vib:
            # print("vib_std_floor",vib_std_floor)
            self.vib = VIB(dim, vib_lat,vib_std_floor); dim = vib_lat
...
def main(a):
    ...
    model = PrivacyNet(base, n_spk,
                       ...
                       null_rank=a.null_rank,vib_std_floor=a.vib_std_floor).to(DEVICE)
...
if __name__ == "__main__":
    ...
    p.add_argument('--beta_vib', type=float, default=2.5e-2, help='Weight for KL term')
    p.add_argument('--vib_std_floor', type=float, default=0.02, help='Minimum std per VIB latent dim (0 disables).Prevents near-deterministic "secret" ID channels.')
    ...
    main(args)
     ...
     # nohup python ../apm0046210-synthetic-speech-detection-train-main/ssd_train/train_vib_w2v2bert.py --data_path "../data/ASVspoof2019_LA/" "../data/in_the_wild/" "../data/ASVspoof2021_LA_eval/" "../data/wavefake/" "../data/LJSpeech-1.1/" --train True --batch_size 72 --epochs 5 --use_vib True --vib_lat 256 --amp_dtype bf16 --dataset_name "combined_hybrid_vib_latent256_beta5e3_e5_wt_for-norm" >combined_hybrid_vib_latent256_beta5e3_e5_wt_for-norm.log &

    #  nohup python ../apm0046210-synthetic-speech-detection-train-main/ssd_train/train_vib_w2v2bert.py --data_path "../data/ASVspoof2019_LA/" "../data/in_the_wild/" "../data/ASVspoof2021_LA_eval/" "../data/wavefake/" "../data/LJSpeech-1.1/" --train True --batch_size 72 --epochs 5 --use_vib True --vib_lat 256 --amp_dtype bf16 --dataset_name "combined_hybrid_vib_latent256_beta2.5e2_vib_std_floor0.02_e5_wt_for-norm" >combined_hybrid_vib_latent256_beta2.5e2_vib_std_floor0.02_e5_wt_for-norm.log &
