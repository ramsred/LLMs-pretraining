...
class AudioDataset(Dataset):
    """
    Each dataset instance builds its own speaker-to-index map so DataLoader
    workers never race on a shared global dict (fixes KeyError on unseen
    speakers like 'Donald Trump').
    """

    def __init__(self, ids, meta, cut=64000):
        self.ids, self.meta, self.cut = ids, meta, cut

        # Build mapping only from bona-fide (label==1) clips
        self.spk2idx = {}
        for k in ids:
            if meta[k]["label"] == 1:
                spk = meta[k]["speaker"].strip()
    ...
    def __getitem__(self, idx):
        ...
        # Speaker index (-1 for synthetic)
        if y == 1:
            spk_name = self.meta[key]["speaker"].strip()
            if spk_name not in self.spk2idx:
                # Shouldn’t happen, but keep it safe & visible
                logging.warning("New bona-fide speaker %s encountered after init", spk_name)
                self.spk2idx[spk_name] = len(self.spk2idx)
            spk = self.spk2idx[spk_name]
        else:
            spk = -1
...
#         for k in ids:
#             print("K",k)
#             if meta[k]["label"] == 1:     # keep bonafide speakers only
#                 spk = meta[k]["speaker"]
...
#                 if y == 1 else -1)        # -1 for synthetic clips
...
def split_dict(meta, tr=0.70, va=0.10, te=0.20, seed=None):
    if seed is not None:
        random.seed(seed)
...
def get_combined_loader(paths, seed, bs):
    ...
    tr_lo = DataLoader(tr_ds, bs, shuffle=True, pin_memory=True, num_workers=8,worker_init_fn=seed_worker, generator=g)
    va_lo = DataLoader(va_ds, bs, shuffle=True, pin_memory=True, num_workers=8,worker_init_fn=seed_worker, generator=g)
