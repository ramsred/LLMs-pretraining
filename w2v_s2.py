...
if __name__ == "__main__":
    ...
    # --------- parse sigma schedule ---------                              
    try:
        sig_list = [float(x) for x in args.sigma.split(",")]
    except ValueError:
        raise ValueError("--sigma must be float or comma list")

    if len(sig_list) == 1:
        sig_list *= args.epochs            # broadcast
    elif len(sig_list) != args.epochs:
        raise ValueError(
            f"λ schedule length {len(sig_list)} "
            f"does not match --epochs {args.epochs}"
        )
    args.sigma_schedule = sig_list
    main(args)
...
# nohup python ../apm0046210-synthetic-speech-detection-train-main/ssd_train/finetune_with_ckpt_grl_s2.py --data_path "../data/ASVspoof2019_LA/" "../data/in_the_wild/" "../data/ASVspoof2021_LA_eval/" "../data/wavefake/" "../data/LJSpeech-1.1/" --train True --batch_size 72 --epochs 2 --lambda_adv 2,2 --use_grl True --amp_dtype bf16 --checkpoint combined_hybrid_lambda_schdule_deep_speaker_network_4layer_e5_relu_b72__wt_for-norm_08_04_ep4_20250804_234017.pth --dataset_name "combined_hybrid_lambda_schdule_deep_speaker_network_4layer_e2_relu_b72__wt_for-norm_s2_08_05" >combined_hybrid_lambda_schdule_deep_speaker_network_4layer_e2_relu_b72_wt_for-norm_s2_08_05.log &

# nohup python ../apm0046210-synthetic-speech-detection-train-main/ssd_train/finetune_with_ckpt_grl_s2.py --data_path "../data/ASVspoof2019_LA/" "../data/in_the_wild/" "../data/ASVspoof2021_LA_eval/" "../data/wavefake/" "../data/LJSpeech-1.1/" --train True --batch_size 72 --epochs 2 --lambda_adv 2,2 --use_grl True --amp_dtype bf16 --checkpoint combined_hybrid_lambda_schdule_deep_speaker_network_4layer_e5_relu_b72__wt_for-norm_08_04_ep4_20250804_234017.pth --dataset_name "grl_stage2_static_noise_speaker_network_4layer_e2_relu_b72" >grl_stage2_static_noise_speaker_network_4layer_e2_relu_b72.log &
"""   
